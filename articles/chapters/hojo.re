...
//lead{
本章では、Chainlitのカスタムフロントエンド機能と@<code>{send_window_message}を使い、LLMが生成したステージで遊べる物理パズルゲームを実装します。
Chainlitの標準チャットUIを拡張した、チャットと連動するインタラクティブなアプリケーションを作成したい方におすすめの内容となっています。
//}
...
Chainlitの標準チャットUIは、ユーザーが1つのメッセージを送信し、システムが1つの応答を返す、一問一答形式です。
...
@<code>{send_window_message}は、チャットUIの外側にある独自のフロントエンド（以降カスタムフロントエンド）にリアルタイムでデータを送ることができます。
さらに、Chainlitにはカスタムフロントエンドを簡単に構築できる仕組みも用意されているため、標準チャットUIをカスタムフロントエンドに変更することもできます。
...
本章ではこのゲームの実装を通じて、@<code>{send_window_message}によるデータ連携や、カスタムフロントエンドの構築、LLMの出力の安定性を保つ工夫について解説します。
...
チャットで「難しいパズルを作って」や「重力を強くして」と指示するたびに、LLMがステージを再生成し、ゲームステージをリアルタイムで更新させることができます。

//image[hojo-1][完成イメージ]{
//}

本実装では、チャットとゲーム画面を並べたことで、LLMへの指示とその結果を一画面で完結させることができ、ユーザーが直感的にLLMと対話しながらゲームを楽しめるようになっています。
...
チャットUIは、Chainlitの標準チャットUIで、ユーザーはここにテキストを入力してLLMに指示を出します。
ゲーム画面は、カスタムフロントエンドで構築されており、マウス操作で板を配置したり、スタートボタンを押してボールを動かしたりすることができます。
...
@<b>{ファイル}	@<b>{役割}	@<b>{行数}
-----------------
@<code>{app.py}	Chainlitバックエンド	約30行
@<code>{llm.py}	LLM連携・ステージ生成・バリデーション	約180行
@<code>{public/index.html}	ゲームフロントエンド	約300行
//}


=== Chainlitバックエンド

Chainlitのバックエンド処理は、@<code>{app.py}で実装します。
@<code>{app.py}では、ユーザーのチャット入力を受け取り、@<code>{llm.py}を介してLLMにステージ生成を依頼し、生成されたステージを@<code>{send_window_message}でカスタムフロントエンドに送る役割を担います。
...
一方、本章の@<code>{send_window_message}はチャットの@<b>{外}にあるカスタムフロントエンドにデータを送ります。
どちらもChainlitの拡張手法ですが、用途が異なります。

//table[comparison][CustomElementとsend_window_messageの比較]{
@<b>{項目}	@<b>{CustomElement}	@<b>{send_window_message}
-----------------
UIの配置	チャットメッセージ内	チャット外のカスタムフロントエンド
...
LLMとの連携は、@<code>{llm.py}で実装します。
@<code>{llm.py}では、LLMへのステージ生成依頼や、LLMの出力のバリデーションを行う役割を担います。
本実装では、OpenAIのAPIを用いてLLMとやり取りするコードを実装しています。
LLMへのステージの生成依頼は、プロンプトエンジニアリングの要素が強く、LLMにゲームデザイナーになりきってもらうための工夫を盛り込んでいます。

しかし、LLMに常に統一されたフォーマットの出力を求めることは難しく、出力されたステージJSONがゲームとして成立しない場合があります。
そのため、出力結果に対するバリデーションと、生成のリトライの仕組みも実装しています。


=== ゲームフロントエンド

ゲームフロントエンドのHTML・CSS・JavaScriptは、@<code>{public/index.html}で実装します。
@<code>{public/index.html}では、Chainlitの標準チャットUIの読み込み、ゲーム画面のレイアウトや描画、ルートURLからのリダイレクトを行う役割を担います。
具体的には、@<code>{send_window_message}で送られてくるステージJSONを受け取って、物理エンジンであるMatter.jsを用いてゲーム画面に描画します。
また、ルートURLから@<code>{public/index.html}へリダイレクトする仕組みを導入することで、アプリが起動したらすぐにゲーム画面が表示されるようにしています。
...
他にも、@<code>{app.py}では@<code>{cl.user_session}を用いて、「現在のステージ」の情報を保持することで、ゲーム仕様を対話的に調整することを可能にしています。
...
「床の上に配置して」という曖昧な指示では、LLMはゴールを宙に浮かせたり壁に埋めたりすることがありました。
...
ゲームフロントエンドは、@<code>{public/index.html}で実装しています。
Chainlitの標準チャットUIは@<code>{iframe}で読み込んで、あとは完全に独立した画面になっています。
ゲームフロントエンドの役割は、主に以下の3点です。
...
まず、Chainlitの標準チャットUIの代わりに、カスタムフロントエンドを表示する仕組みを実装します。
Chainlitはプロジェクトルートに@<code>{public/}ディレクトリがあると、その中のファイルを@<code>{/public/}パスで自動的に配信します。
この仕様のおかげで、@<code>{public/}ディレクトリに@<code>{index.html}を置くだけで、@<code>{http://localhost:8000/public/index.html}でカスタムフロントエンドにアクセスできるようになります。
...
これを@<code>{iframe}で読み込むことで、メッセージ送受信やMarkdown表示などの機能をそのまま活用することができます。
...
==== send_window_messageで送られたデータを受け取る
...
@<code>{send_window_message}を介して受け取ったステージJSONは、以下の流れでゲームとして動き出します。
...
チャットで「新しいパズルを作って」と言うたびに、このサイクルの起点となる@<code>{send_window_message}を介してデータを受け取り、古いリソースをクリーンアップすることで、まっさらな状態からゲーム画面を構築し直します。
...
チャットUIからカスタムフロントエンドにデータを送れるこの仕組みは、物理パズルに限らず、様々な応用が考えられます。

 * @<b>{可視化ダッシュボード} ― LLMが生成したグラフやチャートをリアルタイムで描画

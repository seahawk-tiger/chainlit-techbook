...
//lead{
LLMがツールを使って複雑な処理をこなすようになった今、応答が返るまでの「待ち時間」はユーザー体験の一つの課題です。@<chap>{uchiyama} では開発者目線での LLM の観察・デバッグを扱いましたが、本章では@<b>{ユーザー目線}に立ち、待ち時間の不安をどう解消するかに焦点を当てます。Chainlit を使い始めていてユーザー体験をさらに良くしたい方はもちろん、Chainlit 以外の LLM アプリケーション開発でも応用できる「待ち時間との向き合い方」を知りたい方にも役立つ内容です。Chainlit の @<code>{cl.Step} を起点に、Markdownによる構造化・@<code>{cl.TaskList} によるタスクの表示・@<code>{cl.Plotly} による可視化の強化・@<code>{asyncio.gather} を活用したメッセージの並行表示という4つのアプローチを紹介します。
//}
//pagebreak

== 背景

LLM の精度向上とツールによる機能拡張が進むにつれ、1回の応答にかかる時間が長くなる傾向があります。処理の過程を何も表示しないと、ユーザーは「本当に動いているのか？」という不安を抱きます。さらに、途中経過が何も見えないまま長い時間待った末に、意図とまったく異なる結果が返ってきた場合、待っていた時間そのものが無駄になってしまいます（@<img>{kotsubo/no_step}）。途中経過が見えていれば、方向がずれた時点で早めに気づいて修正できたかもしれません。本章では、処理の過程をわかりやすく表示する工夫と、待ち時間を有効活用する工夫を紹介します。
